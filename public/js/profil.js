...
function selectProfilGame(id) {
  ...
  const row = document.querySelector('.history-table tbody tr[data-id="' + id + '"]');
  ...
  if (!game.id_estate) {
    ...
    setText('pdet-rent',    game.rent ? Number(game.rent).toLocaleString('fr-FR') + ' €' : '—');
    setText('pdet-charges', game.is_charges_included == 1 ? 'Charges comprises' : 'Hors charges');
    setText('pdet-m2',      game.square_meters ? game.square_meters + ' m²' : '—');
    setText('pdet-room',    game.room ?? '—');
    setText('pdet-chamber', game.chamber ?? '—');
    setText('pdet-floor',   game.floor !== null && game.floor !== undefined
      ? (game.floor == 0 ? 'RDC' : 'Étage ' + game.floor) : '—');
    setText('pdet-adress',      game.adress || '—');
    setText('pdet-city',        game.city   || '—');
    setText('pdet-description', game.description || '—');
  }

  setText('pdet-guess', game.guess ? Number(game.guess).toLocaleString('fr-FR') + ' €' : '—');
  setText('pdet-score', game.game_result !== null && game.game_result !== undefined
