...
function buildEditImgGrid(item) {
  ...
  [1, 2, 3, 4].forEach(function (n) {
    ...
    fileInput.onchange = function (e) {
      previewEditImg(n, e.target.files[0]);
    };
    ...
    uploadBtn.onclick = function () {
      document.getElementById("edit-new-img" + n).click();
    };
...
function previewEditImg(n, file) {
  ...
  reader.onload = function (e) {
    ...
    // Show or create the delete button if missing
...
function selectEstateList(id) {
  ...
  document
    .querySelectorAll(".estate-table tbody tr")
    .forEach((tr) => tr.classList.remove("selected"));
  document
    .querySelector('.estate-table tbody tr[data-id="' + id + '"]')
  ...
  setText(
    "det-rent",
    item.rent ? Number(item.rent).toLocaleString("fr-FR") + " €" : "—",
  );
  setText(
    "det-charges",
    item.is_charges_included == 1 ? "Charges comprises" : "Hors charges",
  );
...
function selectEstateSent(id) {
  ...
  document
    .querySelectorAll(".depose-table tbody tr")
    .forEach((tr) => tr.classList.remove("selected"));
  document
    .querySelector('.depose-table tbody tr[data-id="' + id + '"]')
    .classList.add("selected");
...
const API_URL =
  "https://wgbqsvxvnheukskrnlzb.supabase.co/rest/v1/Logement?apikey=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndnYnFzdnh2bmhldWtza3JubHpiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzMyNDExMDMsImV4cCI6MjA4ODgxNzEwM30.iRldHE5p3RuUqVWnR5MftBrysd_7c4BkAqHvYyR4ru8";
...
function fetchBiens() {
  ...
  fetch(API_URL)
    ...
    .then((data) => {
      ...
      const already = apiData.filter((d) =>
        importedIds.includes(Number(d.ID)),
      ).length;
      status.textContent =
        apiData.length + " biens récupérés — " + already + " déjà importé(s).";
...
function selectBien(id) {
  ...
  const isCharges = (item.charges_comprises || "")
    .toLowerCase()
    .includes("comprises")
    ? "1"
    : "0";
  ...
  ["image", "image2", "image3", "image4"].forEach((key, i) => {
    ...
    if (url) {
      ...
      const ph = document.createElement("div");
      ph.className = "api-img-placeholder";
      ph.textContent = "Aucune image";
      card.appendChild(ph);
    }
...
function updateImgCountLabel(selected, available) {
  document.getElementById("img-count-label").textContent =
    available > 0
      ? `(${selected} / ${available} sélectionnée${selected > 1 ? "s" : ""})`
      : "";
}
...
function selectUser(id) {
  ...
  document
    .querySelectorAll(".user-table tbody tr")
    .forEach((tr) => tr.classList.remove("selected"));
  const row = document.querySelector(
    '.user-table tbody tr[data-id="' + id + '"]',
  );
...
function selectGame(id) {
  ...
  document
    .querySelectorAll(".game-table tbody tr")
    .forEach((tr) => tr.classList.remove("selected"));
  document
    .querySelector('.game-table tbody tr[data-id="' + id + '"]')
    .classList.add("selected");
  ...
  setText(
    "gdet-guess",
    item.guess ? Number(item.guess).toLocaleString("fr-FR") + " €" : "—",
  );
  setText(
    "gdet-score",
    item.game_result !== null && item.game_result !== undefined
      ? item.game_result + " pts"
      : "—",
  );
...
function selectMessage(id) {
  ...
  document
    .querySelectorAll(".message-table tbody tr")
    .forEach((tr) => tr.classList.remove("selected"));
  document
    .querySelector('.message-table tbody tr[data-id="' + id + '"]')
    .classList.add("selected");
